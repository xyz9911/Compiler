...
class SemanticAnalyzer:
    def __init__(self, start, new_start='S', point='.', sharp='$', acc='acc'):
        ...
        self.productions = {
            ...
            # 改动
            'Operate': [['0', 'Operator', 'Num', 'do:operateVarNum'], ['Num', 'Operator', 'Num', 'do:operateVarVar'], ['0', '211'],
                        ['0', '214']],
            ...
            'E': [['4', '125', 'ProcessStc', '126', 'do:backPatchElse'], ['null', 'do:backPatchElse']],
            ...
            'LoopStc': [['5', '121', 'Condition', '122', '125', 'ProcessStc', '126', 'do:goBack']
    ...
    def analyze_grammar(self, code):
        ...
        while True:
            tmp = code[pointer]
            action = self.action[status_stack[-1]][str(tmp[0])]
            if action == '':
            ...
            if action[0] == 's':
                ...
                for i in range(0, len(right)):
                    ...
                    if str(symbol_stack[-1].symbol[0]) != right[len(right) - i - 1]:
                        ...
                        elif self.check_can_be_null(current_symbol):
                            if len(self.productions[current_symbol]) == 1:
                                for item in self.productions[current_symbol][0]:
                                    if item[0] == 'd':
                                        self.semantic_action(symbol_stack, item)
                        else:
    ...
    def semantic_action(self, symbol_stack, action):
        if action == 'do:allocateParam':
            ...
            if symbol_stack[-3].symbol[1] not in self.symbol_table.keys():
                ...
                tmp = self.symbol_table[symbol_stack[-3].symbol[1]]
                ...
                if symbol_stack[-1].info.type != symbol_stack[-3].info.type:
                    ...
                    self.symbol_table[symbol_stack[-3].symbol[1]] = (self.symbol_table[symbol_stack[-3].symbol[1]][0],self.symbol_table[symbol_stack[-3].symbol[1]][1],symbol_stack[-3].info.val)
            ...
            if symbol_stack[-3].symbol[1] not in self.symbol_table.keys():
                ...
                if symbol_stack[-1].info.type != symbol_stack[-3].info.type:
                    ...
                    self.tmp_count += 1
                    self.gen_code('t'+str(self.tmp_count)+'='+symbol_stack[-3].symbol[1]+symbol_stack[-2].info.name+str(symbol_stack[-1].info.val))
            ...
            if symbol_stack[-3].symbol[1] not in self.symbol_table.keys() or symbol_stack[-1].symbol[1]:
                ...
                if symbol_stack[-1].info.type != symbol_stack[-3].info.type:
                    ...
                    self.tmp_count += 1
                    self.gen_code(
                        't' + str(self.tmp_count) + '=' + symbol_stack[-3].symbol[1] + symbol_stack[-2].info.name + symbol_stack[-1].symbol[1])
        elif action == 'do:+':
            ...
            if symbol_stack[-3].symbol[1] not in self.symbol_table.keys():
                ...
                if symbol_stack[-1].info.type != symbol_stack[-3].info.type:
                    ...
                    self.gen_code('if '+symbol_stack[-3].symbol[1]+symbol_stack[-2].info.name+str(symbol_stack[-1].info.val)+' goto')
                    self.gen_code('goto ')
            ...
            self.gen_code('goto '+str(symbol_stack[-5].info.quad))
            current_cursor = len(self.code)
            self.back_patch(symbol_stack[-5].info.falseList, current_cursor)
